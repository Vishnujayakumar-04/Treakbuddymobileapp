...
export default function AdventureScreen({ navigation }: AdventureScreenProps) {
  ...
  const handlePlacePress = (place: AdventurePlace) => {
    const placeForDetails: Place = {
      ...
      opening: place.timing || place.timings || (place.openingTimeWeekdays ? `${place.openingTimeWeekdays} - ${place.closingTimeWeekdays}` : 'Open Daily'),
      entryFee: place.price || place.entryFee || 'Varies',
  ...
  const getDisplayDescription = (place: AdventurePlace): string => {
    if (typeof place.description === 'string') {
      ...
      if (place.difficulty) desc += 'Difficulty: ' + place.difficulty + '\n';
      if (place.duration) desc += 'Duration: ' + place.duration + '\n';
      if (place.included && place.included.length > 0) desc += 'Included: ' + place.included.join(', ') + '\n';
      if (place.requirements && place.requirements.length > 0) desc += 'Requirements: ' + place.requirements.join(', ') + '\n';
      if (place.booking) desc += 'Booking: ' + place.booking + '\n';
      return desc.trim();
    }
    ...
    <SafeAreaView style={styles.container}>
      ...
      {!showCategoryView && (
        <View style={{ marginTop: spacing.md }}>
          <FilterPills
            options={['All', ...ADVENTURE_CATEGORIES.map(c => c.id)] as AdventureCategory[]}
            selectedOption={selectedCategory}
            onSelectOption={(val) => setSelectedCategory(val as AdventureCategory)}
          />
        </View>
      )}
